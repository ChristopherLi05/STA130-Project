...
for (i in 1:3) {
  for (j in (i+1):4) {
    ...
    model1 <- lm(score ~ cluster, data = merged_data %>% filter(cluster == i | cluster == j) %>% mutate(cluster =  as.character(cluster)))
...
```{r}
merged_data %>% ggplot(aes(x=cluster, y=score, group=cluster, fill=cluster)) + geom_boxplot()
``` 
